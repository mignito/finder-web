...
export default function ReportPage({ session }) {
  ...
  const [form, setForm] = useState({ name: '', birthdate: '', description: '', reward: '' });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // 생년월일이 있으면 특징 설명 위에 자동으로 함께 저장
      const fullDescription = form.birthdate
        ? `생년월일: ${form.birthdate}\n\n${form.description || ''}`.trim()
        : form.description;
      const { error } = await supabase.from('reports').insert({
        pet_name: form.name, pet_type: '사람', pet_breed: null,
        pet_age: null,
        description: fullDescription, reward: form.reward ? parseInt(form.reward) : 0,
  ...
  return (
    <div className="page">
      <div style={{ padding: '16px 16px 12px', borderBottom: '1px solid var(--border)' }}>
        <h2 style={{ fontSize: 18, fontWeight: 700 }}>실종자 신고 등록</h2>
        <p style={{ fontSize: 12, color: 'var(--text-muted)', marginTop: 4 }}>실종된 사람의 정보를 등록해주세요</p>
      </div>
      ...
      <form onSubmit={handleSubmit}>
        <div onClick={() => fileRef.current.click()}
          ...
          {image
            ...
            : <span style={{ color: 'var(--text-muted)', fontSize: 16 }}>📷 사진 추가</span>}
        </div>
        ...
        <div style={{ padding: '0 16px' }}>
          <div style={{ marginTop: 20 }}>
            ...
            <input className="input" type="text" placeholder="실종자 이름" value={form.name} onChange={set('name')} />
          </div>

          <div style={{ marginTop: 16 }}>
            <label style={{ fontSize: 13, fontWeight: 600, display: 'block', marginBottom: 6 }}>생년월일</label>
            <input className="input" type="date" value={form.birthdate} onChange={set('birthdate')} max={new Date().toISOString().split('T')[0]} />
          </div>

          <div style={{ marginTop: 16 }}>
            <label style={{ fontSize: 13, fontWeight: 600, display: 'block', marginBottom: 6 }}>특징 설명</label>
            <textarea className="input" placeholder="키, 체격, 옷차림, 신체 특징, 마지막 행적 등" value={form.description} onChange={set('description')} rows={4} style={{ resize: 'vertical' }} />
          </div>

          <div style={{ marginTop: 16 }}>
            <label style={{ fontSize: 13, fontWeight: 600, display: 'block', marginBottom: 6 }}>사례금 (원)</label>
            <input className="input" type="number" placeholder="예: 10000" value={form.reward} onChange={set('reward')} />
          </div>

          <div style={{ marginTop: 16 }}>
            <label style={{ fontSize: 13, fontWeight: 600, display: 'block', marginBottom: 6 }}>잃어버린 곳 *</label>
